 ...
 * Project 11
 * By Brooks Jackson
 * Added sound to the collectable and obstacle collisions!
 * Also made it so when the player reaches 120 points (Level 15), they win!
 * Controls:
 * Space = Start
 * W or Up Arrow = UP
 ...
 */
function preload() {
  collectSound = loadSound('assets/teleport.wav');
  deathSound = loadSound('assets/interact.wav');
}
...
function playScreen() {
  ...
  for (let i = 0; i < obstacles.length; i++) {
    if (player.overlap(obstacles[i])) {
      player.remove();
      // Play sound when the player dies
  ...
  for (let i = 0; i < collectables.length; i++) {
    if (player.overlap(collectables[i])) {
      // Play sound when the collectable is collected
      if (collectSound.isLoaded()) {
        collectSound.play();
      }
